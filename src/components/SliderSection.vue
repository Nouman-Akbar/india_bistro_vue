    ...
    <div class="py-16 md:py-20">
      ...
      <div v-if="sectionData.button" class="text-center pt-8">
        <button
          ...
          <span class="slider-action-button__diamond" aria-hidden="true">
            <svg xmlns="http://www.w3.org/2000/svg" width="100%" viewBox="0 0 303 54" fill="none">
              <path
                d="M276.519 54C281.345 44.94 288.288 38.1 293.232 33.9C298.351 29.58 302.529 27.24 303 27C302.47 26.7 298.292 24.3 293.232 20.1C288.23 15.96 281.286 9.06 276.519 0H26.4809C21.6555 9.06 14.7116 15.9 9.7685 20.1C4.64886 24.42 0.470771 26.76 0 27C0.529617 27.3 4.70771 29.7 9.7685 33.9C14.7704 38.04 21.7143 44.94 26.4809 54H276.519Z" />
            </svg>
          </span>
          <span class="slider-action-button__label">{{ sectionData.button.text }}</span>
        </button>
      </div>
    </div>
  </section>
...
const scrollToPrev = () => {
  ...
  state.currentOffset = Math.max(state.currentOffset - step, 0)
}
...
const scrollToNext = () => {
  ...
  const step = cardsWrapper.value.clientWidth * 0.8
  state.currentOffset = Math.min(state.currentOffset + step, state.maxOffset)
}
...
watch(() => state.currentOffset, () => {
  if (horizontalTween) {
    horizontalTween.pause()
  }
